...
if (isset($_POST['submit'])) {
    ...
    $usercheck = "SELECT * FROM signup WHERE username = ?";
    ...
    if ($result->num_rows > 0) { 
        ...
        $sql = "INSERT INTO signup (fname, lname, phone, email, username, password) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        $stmt->bind_param("ssssss", $fname, $lname, $phone, $email, $username, $hashed_password);

        if ($stmt->execute()) {
            $flag = 1;
            $fname = $lname = $phone = $email = ""; // Clear form values
        } else {
